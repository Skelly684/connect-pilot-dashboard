-- Fix the function to queue emails for accepted leads without queued emails
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    v_should_queue := (
      ...
      AND (
        OLD.status IS DISTINCT FROM 'accepted'  -- Status just changed
        OR NEW.next_email_step IS NULL          -- No email queued yet
      )
    );
  END IF;
  ...
  IF v_should_queue THEN
    ...
    END;
  ELSE
    RAISE NOTICE '⏭️ Skipping - should_queue=false (status=%, old_status=%, email=%, next_step=%)', 
      NEW.status, 
...
-- Test on existing accepted lead without queued email
UPDATE public.leads
SET updated_at = NOW()
